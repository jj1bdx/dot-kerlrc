...
OTP_VERSION=$1
OPENSSL_LOCAL="/opt/openssl/1.1.1"
#
env \
    ...
    MAKEFLAGS="-j2" \
    KERL_BUILD_PLT="yes" \
    KERL_CONFIGURE_OPTIONS="--enable-kernel-poll --enable-hipe --enable-dirty-schedulers --enable-sharing-preserving --enable-lock-counter --disable-sctp --disable-dynamic-ssl-lib --with-ssl=${OPENSSL_LOCAL} --without-javac --without-odbc --disable-silent-rules --with-wx-config=/usr/bin/wx-config" \
